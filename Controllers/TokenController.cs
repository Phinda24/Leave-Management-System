...
namespace Leave_Management_system.Controllers
{
    ...
    public class TokenController : Controller
    {
        ...
        public async Task<IActionResult> Register([FromForm] string token)
        {
            ...
            // FIXED: Proper role detection with multiple possible role names
            string collection = "Employees"; // default
            ...
            if (role.Equals("Supervisor", StringComparison.OrdinalIgnoreCase))
                collection = "Supervisors";
                     ...
                     role.Equals("HODs", StringComparison.OrdinalIgnoreCase) ||
                     role.Equals("HeadOfDepartment", StringComparison.OrdinalIgnoreCase) ||
                     role.Equals("Hod", StringComparison.OrdinalIgnoreCase))
                collection = "HODs"; // Make sure this matches your Firestore collection name

            var docRef = _db.Collection(collection).Document(userId);

            // DEBUG: Log what's happening
            Console.WriteLine($"DEBUG: Role='{role}', UserId='{userId}', Collection='{collection}'");
        ...
        public IActionResult DebugSession()
        {
            var sessionData = new
            {
                ...
                SupID = HttpContext.Session.GetString("SupID"),
                HodID = HttpContext.Session.GetString("HodID"),
